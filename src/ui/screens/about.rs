...
pub(super) fn render(frame: &mut Frame) {
    ...
    frame.render_widget(Paragraph::new("ART".dim()).centered(), art_area);
    ...
    frame.render_widget(
        Paragraph::new(text_lines).left_aligned().on_cyan(),
        top_text,
    );
    ...
    frame.render_widget(Paragraph::new(rows).on_light_cyan(), bottom_table);
